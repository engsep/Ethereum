...
const { PUBLIC_KEY, PRIVATE_KEY, BLOCKCHAIN_URL, PORT = 3009, SMARTCONTRACTADDRESS, CHAINID } = process.env;
...
app.post('/certify', async function (req, res) {
    ...
    var metadata = String(req.body.metadata);

    if(metadata){
        var data = PoEContract.methods.certify(document, metadata).encodeABI();
    } else {
        var data = PoEContract.methods.certifyDocumentOnly(document).encodeABI();
    }
...
app.post('/cancel', async function (req, res) {
    ...
    if(metadata){
        var data = PoEContract.methods.cancel(document, metadata).encodeABI();
    } else {
        var data = PoEContract.methods.cancelDocumentOnly(document).encodeABI();
    }
...
app.post('/check', async function (req, res) {
    ...
    var method = PoEContract.methods.check(document);
...
app.post('/notarize', async function (req, res) {
    ...
    if(metadata){
        var data = PoEContract.methods.notarize(hash, metadata).encodeABI();
    } else {
        var data = PoEContract.methods.notarizeHashOnly(hash).encodeABI();
    }
...
app.post('/nullify', async function (req, res) {

    var hash = String(req.body.hash);
    var metadata = String(req.body.metadata);

    if(metadata){
        var data = PoEContract.methods.nullify(hash, metadata).encodeABI();
    } else {
        var data = PoEContract.methods.nullifyHashOnly(hash).encodeABI();
    }
...
app.post('/getHistory', async function (req, res) {
    ...
    var method = PoEContract.methods.getHistory(hash);
...
app.post('/verify', async function (req, res) {
    ...
    var method = PoEContract.methods.verify(hash);
